...
export default function Preview({creation, onEdit, onRecover, submitting}){
 ...
 {(pending||versions.at(-1)?.status==='failed')&&<div className="notice" role="status"><span>{versions.at(-1)?.message}</span>{pending&&!creation.busy&&<button className="button" disabled={submitting} onClick={onRecover}><RefreshCw size={15}/>查询进度</button>}</div>}
